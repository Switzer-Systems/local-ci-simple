...
The selected-workflow branch ref and target revision are different controls. The runner-group policy controls **which workflow may acquire the runner**. The trusted workflow separately fails closed unless the operator-supplied expected SHA equals the workflow run's exact `github.sha` on `refs/heads/main`.
...
- exact actor and triggering actor `wswitzer`;
...
4. Administrator/automation bypass is disabled or otherwise explicitly bounded so the trusted workflow cannot be silently replaced outside the reviewed path.
5. Force pushes and branch deletion are disabled.
6. Required deterministic checks for the trust contract are preserved.
...
- `main` protection satisfies the approved sandbox configuration;
